...
export const InventarioAdmin: React.FC<InventarioAdminProps> = ({ user }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-bold text-xs font-sports uppercase tracking-wider rounded-xl shadow-lg flex items-center gap-2 self-start sm:self-auto cursor-pointer"
          >
      ...
      {feedbackMessage && (
        <div className="p-3 bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 rounded-xl text-xs font-bold flex items-center justify-between font-body">
          <span className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-400" />
            {feedbackMessage}
          </span>
          <button onClick={() => setFeedbackMessage(null)} className="text-emerald-400 hover:underline cursor-pointer">
            Cerrar
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[#0F1626] p-4 rounded-2xl border border-slate-700/80 shadow-lg flex items-center gap-3.5">
          <div className="p-3 rounded-xl bg-blue-950/60 text-blue-400 border border-blue-500/30">
            <Package className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs text-slate-400 font-sports uppercase tracking-wider">Unidades en Stock</p>
            <p className="text-2xl font-scoreboard font-bold text-white tracking-wider">{totalStockUnits.toLocaleString()} <span className="text-sm font-sports uppercase text-slate-400 font-normal">pzas</span></p>
          </div>
        </div>

        <div className="bg-[#0F1626] p-4 rounded-2xl border border-slate-700/80 shadow-lg flex items-center gap-3.5">
          <div className="p-3 rounded-xl bg-emerald-950/60 text-emerald-400 border border-emerald-500/30">
            <DollarSign className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs text-slate-400 font-sports uppercase tracking-wider">Valor Total de Inventario</p>
            <p className="text-2xl font-scoreboard font-bold text-white tracking-wider">${totalInventoryValue.toLocaleString('es-MX')} <span className="text-sm font-sports uppercase text-red-400 font-normal">MXN</span></p>
          </div>
        </div>

        <div className="bg-[#0F1626] p-4 rounded-2xl border border-slate-700/80 shadow-lg flex items-center gap-3.5">
          <div className={`p-3 rounded-xl ${lowStockCount > 0 ? 'bg-amber-950/60 text-amber-400 border border-amber-500/30' : 'bg-slate-800 text-slate-400'}`}>
            <AlertTriangle className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs text-slate-400 font-sports uppercase tracking-wider">Alertas de Stock Bajo</p>
            <p className={`text-2xl font-scoreboard font-bold tracking-wider ${lowStockCount > 0 ? 'text-amber-400' : 'text-slate-400'}`}>{lowStockCount} <span className="text-sm font-sports uppercase text-slate-400 font-normal">productos</span></p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#0F1626] p-4 rounded-2xl border border-slate-700/80 shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-[#0A0E17] border border-slate-700/80 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-red-500"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            className="px-3 py-2 border border-slate-700/80 rounded-xl text-xs font-sports uppercase tracking-wider bg-[#0A0E17] text-slate-200 focus:outline-hidden focus:border-red-500"
          >
            {categories.map((c) => (
              <option key={c} value={c} className="bg-[#0A0E17] text-white">{c}</option>
            ))}
          </select>

          <button
            onClick={() => setShowLowStockOnly(!showLowStockOnly)}
            className={`px-3 py-2 rounded-xl text-xs font-bold font-sports uppercase tracking-wider transition-colors flex items-center gap-1.5 cursor-pointer ${
              showLowStockOnly
      ...
      {loading ? (
        ...
        <div className="bg-[#0F1626] rounded-2xl border border-slate-700/80 shadow-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-[#0A0E17] text-slate-300 font-bold font-sports uppercase tracking-wider text-[11px] border-b border-slate-800">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800 font-body">
                {filteredProducts.map((prod) => {
                  ...
                  return (
                    <tr key={prod.id} className="hover:bg-[#131A28] transition-colors">
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3">
                          <img
                            src={normalizeGoogleDriveImageUrl(prod.image) || getDefaultProductPlaceholder(prod.category)}
                            alt={prod.name}
                            className="w-10 h-10 rounded-lg object-contain p-0.5 bg-slate-950 shrink-0 border border-slate-700"
                      ...
                      <td className="py-3.5 px-4 font-mono font-semibold text-slate-400">{prod.sku}</td>
                      <td className="py-3.5 px-4 font-medium text-slate-300 font-sports uppercase tracking-wider">{prod.category}</td>
                      <td className="py-3.5 px-4 font-scoreboard font-bold text-white text-base">${prod.price.toLocaleString('es-MX')}</td>
                      <td className="py-3.5 px-4 text-slate-400 font-scoreboard font-bold text-base">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="p-1.5 rounded-lg bg-[#0A0E17] hover:bg-red-950/60 text-slate-400 hover:text-red-400 border border-slate-700/80 transition-colors cursor-pointer"
                            title="Disminuir stock"
                          >
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className={`font-scoreboard font-bold text-lg px-2 min-w-[2.5rem] text-center ${isLow ? 'text-amber-400' : 'text-white'}`}>
                            {prod.stock}
                          </span>
                          <button
                            onClick={() => handleStockDelta(prod.id, 1)}
                            className="p-1.5 rounded-lg bg-[#0A0E17] hover:bg-emerald-950/60 text-slate-400 hover:text-emerald-400 border border-slate-700/80 transition-colors cursor-pointer"
                      ...
                      <td className="py-3.5 px-4">
                        {isLow ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-950/80 border border-amber-500/40 text-amber-300 font-sports uppercase tracking-wider">
                            <AlertTriangle className="w-3 h-3" /> Bajo (Min: {prod.minStockAlert})
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 font-sports uppercase tracking-wider">
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-950/50 transition-colors cursor-pointer"
      ...
      {isModalOpen && editingProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 bg-black/80 backdrop-blur-md overflow-y-auto animate-in fade-in duration-150">
          <div className="bg-[#0F1626] w-full max-w-lg rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-700/80 overflow-hidden flex flex-col max-h-[92vh] sm:max-h-[88vh] my-auto animate-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleSaveProduct} className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-3.5 text-xs text-slate-300">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block font-sports uppercase tracking-wider font-bold text-slate-300 mb-1">Código SKU *</label>
                  <input
                    type="text"
                    required
                    value={editingProduct.sku || ''}
                    onChange={(e) => setEditingProduct({ ...editingProduct, sku: e.target.value })}
                    className="w-full p-2 bg-[#0A0E17] border border-slate-700/80 rounded-xl font-mono font-bold text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-red-500"
                  />
                </div>
                <div>
                  <label className="block font-sports uppercase tracking-wider font-bold text-slate-300 mb-1">Categoría</label>
                  <select
                    value={editingProduct.category || 'Jerseys'}
                    onChange={(e) => setEditingProduct({ ...editingProduct, category: e.target.value as any })}
                    className="w-full p-2 bg-[#0A0E17] border border-slate-700/80 rounded-xl font-sports uppercase tracking-wider text-xs text-white focus:outline-hidden focus:border-red-500"
                  >
                    <option value="Jerseys" className="bg-[#0A0E17] text-white">Jerseys</option>
                    <option value="Gorras" className="bg-[#0A0E17] text-white">Gorras</option>
                    <option value="Sudaderas" className="bg-[#0A0E17] text-white">Sudaderas</option>
                    <option value="Souvenirs" className="bg-[#0A0E17] text-white">Souvenirs</option>
                    <option value="Accesorios" className="bg-[#0A0E17] text-white">Accesorios</option>
                    <option value="Coleccionables" className="bg-[#0A0E17] text-white">Coleccionables</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="block font-sports uppercase tracking-wider font-bold text-slate-300 mb-1">Nombre del Producto *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-2 bg-[#0A0E17] border border-slate-700/80 rounded-xl font-semibold text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-red-500"
                />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block font-sports uppercase tracking-wider font-bold text-slate-300 mb-1">Precio Venta (MXN) *</label>
                  <input
                    type="number"
                    required
                    min="0"
                    ...
                    className="w-full p-2 bg-[#0A0E17] border border-slate-700/80 rounded-xl font-scoreboard text-base font-bold text-white placeholder-slate-500 focus:outline-hidden focus:border-red-500"
                  />
                </div>
                <div>
                  <label className="block font-sports uppercase tracking-wider font-bold text-slate-300 mb-1">Costo Unitario</label>
                  <input
                    type="number"
                    required
                    min="0"
                    ...
                    className="w-full p-2 bg-[#0A0E17] border border-slate-700/80 rounded-xl font-scoreboard text-base font-bold text-slate-300 placeholder-slate-500 focus:outline-hidden focus:border-red-500"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block font-sports uppercase tracking-wider font-bold text-slate-300 mb-1">Alerta Stock Mínimo</label>
                  <input
                    type="number"
                    min="1"
                    value={editingProduct.minStockAlert || 5}
                    onChange={(e) => setEditingProduct({ ...editingProduct, minStockAlert: Number(e.target.value) })}
                    className="w-full p-2 bg-[#0A0E17] border border-slate-700/80 rounded-xl font-scoreboard text-base text-white focus:outline-hidden focus:border-red-500"
                  />
                </div>
                <div>
                  <label className="block font-sports uppercase tracking-wider font-bold text-slate-300 mb-1">Proveedor</label>
                  <input
                    type="text"
              ...
              <div className="p-3.5 bg-[#0A0E17] rounded-2xl border border-slate-700/80 space-y-2.5">
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-start">
                  ...
                  <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-xl overflow-hidden bg-slate-950 border border-slate-700 shrink-0 relative shadow-inner flex items-center justify-center">
                    <img
                  ...
                  <div className="flex-1 space-y-1.5 w-full">
                    ...
                    <input
                      ...
                      className="w-full p-2 bg-[#0F1626] border border-slate-700/80 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-red-500"
                    />
              ...
              <div>
                <label className="block font-sports uppercase tracking-wider font-bold text-slate-300 mb-1">Descripción</label>
                <textarea
                  rows={2}
