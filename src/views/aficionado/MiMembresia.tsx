...
export const MiMembresia: React.FC<MiMembresiaProps> = ({ user }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#0F1626] rounded-2xl border border-slate-700/80 p-8 sm:p-12 text-center space-y-5 shadow-xl font-sports">
        <div className="w-16 h-16 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-400 flex items-center justify-center mx-auto shadow-inner">
          <Lock className="w-8 h-8" />
        </div>

        <div className="max-w-lg mx-auto space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30 tracking-wider uppercase">
          ...
          <p className="text-xs sm:text-sm text-slate-300 leading-relaxed font-sans">
            ...
            <strong className="text-white">{storeProfile.stadiumName}</strong> ({storeProfile.teamName})
            ha sido temporalmente pausada por la administración del recinto.
          </p>
        </div>

        {/* Datos de Vinculación Confirmada con el Estadio */}
        <div className="max-w-md mx-auto p-4 bg-[#0A0E17] border border-slate-700/80 rounded-xl text-left space-y-2.5">
          <div className="flex items-center justify-between text-xs font-bold text-slate-200 border-b border-slate-800 pb-2">
            <span className="flex items-center gap-1.5 text-white">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" /> Enlace de Datos Activo
            </span>
            <span className="text-[11px] font-mono text-slate-400">{activeVenueId || 'venue-teodoro-mariscal'}</span>
          </div>

          <div className="grid grid-cols-2 gap-2 text-xs text-slate-300 pt-1 font-sans">
            <div>
              <span className="block text-[10px] uppercase font-bold text-slate-400 font-sports">Recinto Actual</span>
              <span className="font-semibold text-white">{storeProfile.stadiumName}</span>
            </div>
            <div>
              <span className="block text-[10px] uppercase font-bold text-slate-400 font-sports">Franquicia</span>
              <span className="font-semibold text-white">{storeProfile.teamName}</span>
            </div>
          </div>
        </div>
